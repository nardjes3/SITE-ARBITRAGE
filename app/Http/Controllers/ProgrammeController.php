...
public function updateBySemaine(Request $request, $semaine)
{
    ...
    return redirect()->route('index')->with('success', 'Les programmes pour la semaine '.$semaine.' ont été mis à jour avec succès.');
}
public function editBySemaine($semaine)
{
    $programmes = Programme::where('semaines', $semaine)->get();
    return view('edit', compact('programmes', 'semaine'));
}
